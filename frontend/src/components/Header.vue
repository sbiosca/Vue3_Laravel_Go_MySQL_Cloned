    ...
    <nav id="header_nav" class="header_nav">
        ...
        <div>
            ...
            <router-link class="nav-link" to="/profile" v-if="state.isToken">
                <span class="spanProfile"  title="Profile">Profile</span><br>
                <img :src="state.data.user.avatar" class="w-25" style="width: 20%;"/>
            </router-link>
            <router-link class="nav-link" to="/login" v-if="!state.isToken" >
                <font-awesome-icon  icon="fa-solid fa-user" title="Login" size="2x" class="span" />
            </router-link>
            <button class="logout" @click="logout" v-if="state.isToken">
                <font-awesome-icon  icon="fa-solid fa-right-from-bracket" title="DASHBOARD" size="2x" class="span" />
            </button>
        </div>
    </nav>
</template>
